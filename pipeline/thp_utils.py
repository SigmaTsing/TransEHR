...
from THP.Models import get_non_pad_mask
...
def log_likelihood(model, data, time, types):
    ...
    # print(np.sum(np.isnan(all_lambda.detach().cpu().numpy()), axis = -1))
    event_ll = compute_event(type_lambda, non_pad_mask)
    # print(np.sum(np.isnan(event_ll.detach().cpu().numpy()), axis = -1))
    event_ll = torch.sum(event_ll, dim=-1)
    # print(np.sum(np.isnan(event_ll.detach().cpu().numpy())))
